...
** 属性动画
   顾名思义，就是随时间不断变化对象的 *属性* 。属性的变化方式最终影响对象的动画表现。一般而言，属性变化直接依赖 *插值函数* 。常见的匀速（线性）变化，实质上就是 ~y = k*x~ 。更灵活的变化方式，当属bezier变化。bezier起初是为了计算机绘制平滑曲线而发明了bezier多项式参数曲线。插值函数的作用在于可以让计算机根据函数的计算结果来控制属性变化的速率。常见的变化有线性变化和缓动变化，两者都可以用二维bezier曲线函数还表示。可以看看[[cubic-bezier.com][这个在线演示]]。
